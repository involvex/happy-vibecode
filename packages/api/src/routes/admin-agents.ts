...
adminAgentsRouter.use('*', adminMiddleware)

adminAgentsRouter.get('/', async c => {
	const db = createDb(c.env.DB)
	// Return ALL agents (not filtered by isActive) for admin management
...
adminAgentsRouter.post('/', async c => {
	const db = createDb(c.env.DB)
...
adminAgentsRouter.put('/:id', async c => {
	const db = createDb(c.env.DB)
...
adminAgentsRouter.delete('/:id', async c => {
	const db = createDb(c.env.DB)
	const id = c.req.param('id')
